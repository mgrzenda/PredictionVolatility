...
public class InstanceInProgress {
	...
	public void mapPredictionsToBins(BinnedPerformanceEvaluator<Example<Instance>>[] binEvaluators) {
		...
		double[] votesForFirstPrediction=null;
		...
		for (int i = 0; i < predictionCount; i++) {
			...
			case FIRST_PREDICTION:
				...
				votesForFirstPrediction=predictions.get(i).getClassVotes();
				break;

				// prediction made in test-then-train mode i.e. right before
				// receiving true label
			case FINAL_PREDICTION:
				predictions.get(i).setBinIndex(binCount + 1);

				assert votesForFirstPrediction!=null : "Missing first time prediction";

				// add new prediction and calculate differential measures
				binEvaluators[binCount + 1].addDifferentialResult(this.getExample(), votesForFirstPrediction, predictions.get(i).getClassVotes());

				// replaced with the line above binEvaluators[binCount + 1].addResult(this.getExample(), predictions.get(i).getClassVotes());
